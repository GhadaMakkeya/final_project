import 'dart:ui';

class AppColors {
  static const Color backgroundColor = Color(0xffF2EDE8);
  static const Color primaryColor = Color(0xFF1B2A4A);
  static const Color secondaryColor = Color(0xffD9B36E);
  static const Color productCardColor = Color(0xffF2EDE8);
  static const Color authCardColor =  Color.fromARGB(255, 253, 250, 250);
  static const Color white = Color(0xffF5EDE4);
  static const Color grey = Color(0xff808080);
  static const Color black = Color(0xff000000);
  static const Color textHelper = Color(0xffD1C5B4);
  static const Color red = Color(0xffFF0000);

  static const Color imageBackground = Color(0xFFFBF9F8);
  static const Color white_ = Color(0xFFFFFFFF);

  static const Color textPrimary = Color(0xFF1B1C1C);
  static const Color textSecondary = Color(0xFF565E77);

  static const Color accentGold = Color(0xFF775A1E);

  static const Color borderLight = Color(0xFFE4E2E2);

  // -- add product field colors --
  static const Color brown = Color(0xFF4E4639);
  static const Color brownLight = Color.fromARGB(255, 121, 95, 68);
  static const Color formFieldFill = Color.fromARGB(255, 251, 248, 245);

  
}
